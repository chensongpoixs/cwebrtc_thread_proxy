...
namespace chen
{
	...
	namespace internal {
		...
		public:
			...
			void Invoke(const Location& posted_from, Thread* t);
	...
	public:
		...
		R Marshal(const chen::Location& posted_from, Thread* t) {
	...
	public:
		...
		R Marshal(const  Location& posted_from, Thread* t ) {
			internal::SynchronousMethodCall(this).Invoke( posted_from  , t );
			return r_.moved_result();
		}
...
#define PROXY_WORKER_METHOD1(r, method, t1)												\
  r method(t1 a1)  {																	\
	GET_OBJECT_THREAD(a1)																\
     ...
     return call.Marshal(CHEN_FROM_HERE, object->m_thread);								\
   }
